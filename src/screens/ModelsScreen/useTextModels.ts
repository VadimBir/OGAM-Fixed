...
function computeFilteredResults(
  ...
  const filtered = searchResults.filter(model => {
    ...
    // Do NOT hide models whose files exceed the RAM budget: the user explicitly wants to
    // see and download any GGUF (they judge their own headroom / use Load Anyway). The
    // per-model memory warning still flags over-budget models at selection/load time.
    return true;
  });
...
export function useTextModels(setAlertState: (s: AlertState) => void) {
  ...
  const filteredResults = useMemo(
    () => applySort(computeFilteredResults(searchResults, filterState), filterState.sort, ramGB),
