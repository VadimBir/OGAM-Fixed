...
/**
 * When a turn fails with a deterministic PROGRAMMING/native-binding fault ("undefined is not a
 * function", "Cannot read property … of undefined") the bare message is useless for diagnosis — it
 * names no call site. Static analysis has cleared every JS name in the generation path, so the real
 * culprit is a runtime-undefined symbol (a native/newArch binding, or a circular-init export) that
 * only the STACK identifies. Append the error class + the first few stack frames so the failed turn
 * itself names the throwing call (readable in debug builds; a bundle offset in release). Non-program
 * errors (context-full, model busy, native decode) are returned unchanged — they already read clearly.
 */
function describeGenerationError(error: any, msg: string): string {
  const isProgramError =
    error instanceof TypeError ||
    error instanceof ReferenceError ||
    /is not a function|is not an object|cannot read propert/i.test(msg);
  if (!isProgramError) return msg;
  const frames = typeof error?.stack === 'string'
    ? error.stack.split('\n').slice(0, 4).map((l: string) => l.trim()).join('\n')
    : '';
  const cls = error?.name && !msg.startsWith(error.name) ? `${error.name}: ` : '';
  return frames ? `${cls}${msg}\n\n[diag]\n${frames}` : `${cls}${msg}`;
}
  ...
  try {
    ...
    if (isContextOverflow) {
      ...
      deps.addMessage(targetConversationId, {
        role: 'assistant',
        content: describeGenerationError(error, msg),
        turnStatus: 'failed',
      });
      deps.setAlertState(
        showAlert(
