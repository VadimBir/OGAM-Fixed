...
export interface ImageGenerationParams {
  ...
  useOpenCL?: boolean;
  /**
   * img2img reference image as base64 (raw RGB or PNG bytes, no data-URL prefix). When set,
   * the request becomes img2img. Only takes effect on the MNN/CPU backend with a VAE encoder
   * present (the SD core disables img2img otherwise).
   */
  initImage?: string;
  /**
   * img2img denoise amount in 0..1: how much the reference is changed. 1.0 = ignore the ref
   * (pure txt2img), low values stay close to the ref image. Default 0.6 when an initImage is set.
   */
...
export interface Project {
  ...
  icon?: string;
  /**
   * Optional image avatar — a `file://` path or a `data:` URI. When set, character/persona rows,
   * the selector, and chat messages render this instead of the `icon` color. Populated on
   * SillyTavern PNG import (the imported card image) or a user-picked image.
   */
  avatarUri?: string;
  /**
   * Character-card metadata (SillyTavern `chara_card_v2`). All optional: a plain persona leaves
   * them unset and behaves exactly as before. On import these map from
   * personality / scenario / first_mes / mes_example and are folded into the assembled system
   * prompt so the model stays in character.
   */
  personality?: string;
  scenario?: string;
  /** First message the character sends when a new chat starts (SillyTavern `first_mes`). */
  firstMessage?: string;
  /** Few-shot example dialogue (SillyTavern `mes_example`). */
  exampleDialogue?: string;
  /**
   * Skill/tool card ids this character enables. When set (non-undefined) it overrides the global
   * enabled-tools selection for chats under this character; unset ⇒ inherit the global selection.
   */
