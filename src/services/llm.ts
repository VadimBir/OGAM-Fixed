...
class LLMService {
  ...
  private async validateAndPrepareModel(modelPath: string, override: boolean = false): Promise<{ fileSize: number; memCheck: Awaited<ReturnType<typeof checkMemoryForModel>>; params: ReturnType<typeof buildModelParams> }> {
    ...
    // Model size + total RAM gate the mmap-off (repack speed) path: disabling mmap loads the
    // whole model into non-reclaimable RAM and OOM-kills the app for all but small models.
    const deviceTotalMemoryGB = hardwareService.getTotalMemoryGB();
    const params = buildModelParams(modelPath, { ...settings, nThreads: effectiveNThreads, speculativeDecoding, modelFileSizeBytes: fileSize, deviceTotalMemoryGB });
    logger.log(`[LLM] Resolved params: threads=${params.nThreads}, batch=${params.nBatch}, ctx=${params.ctxLen}, gpuLayers=${params.nGpuLayers}, mmap=${(params.baseParams as { use_mmap?: boolean }).use_mmap}`);
  ...
  async generateResponse(messages: Message[], options?: { onStream?: StreamCallback; onComplete?: CompleteCallback; disableThinking?: boolean }): Promise<string> {
    ...
    const completionWork = (async () => {
      ...
      // [JSI-TRACE] Under the New Architecture, llama.rn's completion() is a JSI-bound method. If the
      // JSI host-object was not installed, ctx.completion is `undefined` here even though the model
      // loaded, and calling it throws "undefined is not a function". Log the resolved TYPE of each ctx
      // method so the debug log names EXACTLY which native binding is missing (or confirms all present).
      const ctxAny = ctx as any;
      logger.log(
        `[JSI-TRACE] ctx bindings — completion=${typeof ctxAny.completion} ` +
        `tokenize=${typeof ctxAny.tokenize} detokenize=${typeof ctxAny.detokenize} ` +
        `getFormattedChat=${typeof ctxAny.getFormattedChat} stopCompletion=${typeof ctxAny.stopCompletion} ` +
        `clearCache=${typeof ctxAny.clearCache} id=${String(ctxAny.id)} gpu=${String(ctxAny.gpu)}`,
      );
      if (typeof ctxAny.completion !== 'function') {
        throw new Error(
          `[JSI-TRACE] ctx.completion is ${typeof ctxAny.completion} (not a function) — llama.rn JSI ` +
          `binding not installed under the New Architecture. Model loaded but cannot generate.`,
        );
      }
